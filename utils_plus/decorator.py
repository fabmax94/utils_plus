...
class lazy_property(object):
    ...
    def __get__(self, obj, type_):
        ...
        val = _function_execution(lambda: self.function(obj), func_name=self.function.__name__)
        obj.__dict__[self.function.__name__] = val
        return val


def _function_execution(function, func_name="function_execution"):
    try:
        result = function()
...
def notify_thread(argument):
    def decorator(function):
        def wrapper(*args, **kwargs):
            result = _function_execution(lambda: function(*args, **kwargs), func_name=function.__name__)
...
def timeout(time_count):
    def decorator(function):
        def wrapper(*args, **kwargs):
            ...
            def process(resp):
                resp["result"] = _function_execution(lambda: function(*args, **kwargs), func_name=function.__name__)
...
def measure_time(callback):
    def decorator(function):
        def wrapper(*args, **kwargs):
            ...
            callback(args[0], round(delta_in_minutes, 2))
            return result
